...
def load_model(model_name, label_num, hidden_size, num_layers, model_path):
    model = eval(f"{model_name}({label_num}, {hidden_size}, {num_layers})").to(device)
    checkpoint = torch.load(os.path.join(ROOT_PATH, model_path), weights_only=True, map_location=device)
...
if __name__ == "__main__":
    ...
    model_path0 = r"./ckpt/torch/LY_epoch_1867_loss_0.002432126324856654.pt"
    ...
    with mss.mss() as sct:
        ...
        while "Screen capturing":
            ...
            if crop_image is None:
                ...
                # nn inference
                dino_image = torch.tensor(dino_image).to(torch.float32).unsqueeze(0).unsqueeze(0).to(device)

                nn_out0, h_0, c_0 = model0.real_time_forward(dino_image, h_0, c_0)

                inference_result = int(torch.argmax(nn_out0))
                # nn result
